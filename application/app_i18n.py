...
CATALOG: dict[str, Any] = {
    ...
    "mega_backdoor": {
        "title": {"en": "Mega Backdoor Roth", "zh": "Mega Backdoor Roth"},
        "subtitle": {"en": "Estimate how much after-tax 401(k) room remains this year for a Mega Backdoor Roth contribution.", "zh": "估算本年度还剩多少税后 401(k) 空间可用于 Mega Backdoor Roth。"},
        "tax_year": {"en": "Tax year", "zh": "税务年度"},
        "age": {"en": "Age at year end", "zh": "年末年龄"},
        "eligible_compensation": {"en": "Plan-eligible compensation", "zh": "计划认可薪酬"},
        "employee_deferrals": {"en": "Total employee deferrals (pre-tax + Roth 401(k))", "zh": "员工递延总额（税前 + Roth 401(k)）"},
        "employer_contributions": {"en": "Expected employer contributions", "zh": "预计雇主供款"},
        "after_tax_already": {"en": "After-tax contributions already made", "zh": "本年已做税后供款"},
        "catch_up_limit": {"en": "Catch-up limit to use", "zh": "使用的 catch-up 上限"},
        "plan_after_tax_cap": {"en": "Optional plan-specific after-tax cap", "zh": "可选：计划内税后供款上限"},
        "allows_after_tax": {"en": "Plan allows after-tax 401(k) contributions", "zh": "计划允许税后 401(k) 供款"},
        "allows_conversion": {"en": "Plan allows in-plan Roth conversion or rollover", "zh": "计划允许 plan 内 Roth 转换或转出"},
        "available_after_tax": {"en": "Remaining after-tax room", "zh": "剩余税后空间"},
        "annual_additions_cap": {"en": "415(c) cap used", "zh": "使用的 415(c) 上限"},
        "counted_deferrals": {"en": "Deferrals counted toward 415(c)", "zh": "计入 415(c) 的员工递延"},
        "employer_total": {"en": "Employer contributions", "zh": "雇主供款"},
        "calculation_breakdown": {"en": "Calculation breakdown", "zh": "计算拆解"},
        "quick_checks": {"en": "Quick checks", "zh": "快速检查"},
        "catch_up_used": {"en": "Catch-up used", "zh": "已用 catch-up"},
        "compensation_cap": {"en": "IRS compensation cap", "zh": "IRS 薪酬上限"},
        "detail_comp_cap": {"en": "Compensation counted for plan limit", "zh": "计入计划上限的薪酬"},
        "detail_annual_additions_limit": {"en": "IRS annual additions dollar limit", "zh": "IRS 年度总供款金额上限"},
        "detail_effective_415c_limit": {"en": "Effective 415(c) limit used", "zh": "实际使用的 415(c) 上限"},
        "detail_elective_limit": {"en": "Elective deferral limit", "zh": "elective deferral 上限"},
        "detail_counted_deferrals": {"en": "Employee deferrals counted", "zh": "计入的员工递延"},
        "detail_employer_contributions": {"en": "Employer contributions", "zh": "雇主供款"},
        "detail_raw_room": {"en": "Raw after-tax room before YTD after-tax", "zh": "扣除已做税后供款前的原始空间"},
        "detail_after_tax_already": {"en": "After-tax contributions already made", "zh": "已做税后供款"},
        "detail_remaining_room": {"en": "Estimated remaining after-tax room", "zh": "估算剩余税后空间"},
        "after_tax_not_available": {"en": "This plan setting says after-tax 401(k) contributions are not available, so Mega Backdoor Roth is not executable here even if the formula shows theoretical room.", "zh": "按这个计划设置，税后 401(k) 供款不可用，因此即使公式显示有理论空间，也无法实际执行 Mega Backdoor Roth。"},
        "conversion_not_available": {"en": "After-tax room may exist, but the plan does not appear to allow the Roth conversion / rollover step needed to complete a Mega Backdoor Roth.", "zh": "可能存在税后供款空间，但计划似乎不允许完成 Mega Backdoor Roth 所需的 Roth 转换 / 转出步骤。"},
        "looks_executable": {"en": "The plan settings look compatible with a Mega Backdoor Roth, assuming payroll and plan admin rules cooperate.", "zh": "从计划设置看，Mega Backdoor Roth 基本可行，前提是工资系统和计划管理员规则允许。"},
        "about_body": {
            "en": "**1) Concept**\n\nA Mega Backdoor Roth uses after-tax 401(k) contributions and then converts or rolls them into Roth. The practical limit is usually the 415(c) annual-additions cap minus employee deferrals that count toward that cap and minus employer contributions. Catch-up contributions are generally tracked separately.\n\n**2) What this tool is about**\n\nThis page estimates how much current-year after-tax 401(k) room is still available for a Mega Backdoor Roth. It is meant to answer the narrow question: \"How much more can I still put in this year?\"\n\n**3) How to use it**\n\nEnter your tax year, compensation, total employee deferrals, expected employer contributions, and any after-tax money already contributed. If your plan has a lower internal after-tax cap, enter that too. The result is an estimate of the remaining after-tax room, plus a quick feasibility check for whether the plan appears to support the conversion step.",
            "zh": "**1) 概念**\n\nMega Backdoor Roth 的做法通常是：先往 401(k) 里做税后供款，再把这部分资金转换或转出到 Roth。实际限制通常来自 415(c) 年度总供款上限，减去计入该上限的员工递延供款，再减去雇主供款。catch-up 一般单独计算。\n\n**2) 这个工具是做什么的**\n\n这个页面用来估算：本年度还剩多少税后 401(k) 空间可用于 Mega Backdoor Roth。它聚焦回答一个更窄的问题：\"今年我还能再放进去多少？\"\n\n**3) 如何使用**\n\n输入税务年度、计划认可薪酬、员工递延总额、预计雇主供款，以及已经完成的税后供款。如果你的计划还有更低的内部税后上限，也可以填进去。结果会给出剩余税后空间估算，并做一个快速可执行性检查，看看计划是否支持完成转换这一步。"
        },
        "caption": {"en": "Educational estimate only. Real plans can impose payroll timing, per-paycheck, ACP testing, and plan-document restrictions beyond the IRS limits shown here.", "zh": "仅供教育与估算使用。真实计划可能还有发薪周期、每次工资、ACP 测试和计划文件等额外限制，这些都可能比 IRS 上限更严格。"},
        "help": {
            "tax_year": {"en": "Used to load the IRS elective deferral, catch-up, annual-additions, and compensation-cap values for the selected year.", "zh": "用于载入对应年度的 IRS elective deferral、catch-up、年度总供款和薪酬上限。"},
            "age": {"en": "Age at year end determines whether regular catch-up or the SECURE 2.0 higher catch-up could apply.", "zh": "年末年龄决定是否适用常规 catch-up，或 SECURE 2.0 更高 catch-up。"},
            "eligible_compensation": {"en": "Compensation that counts under the plan for contribution limits. The effective 415(c) cap cannot exceed this amount.", "zh": "计划下可用于计算供款上限的薪酬。实际 415(c) 上限不能高于这个金额。"},
            "employee_deferrals": {"en": "Total employee salary deferrals for the year across pre-tax and Roth 401(k). The part up to the elective-deferral limit counts toward 415(c).", "zh": "本年度员工工资递延总额，包含税前和 Roth 401(k)。其中不超过 elective deferral 上限的部分会计入 415(c)。"},
            "employer_contributions": {"en": "Expected employer match, nonelective, or profit-sharing contributions for the full year.", "zh": "预计全年雇主 match、nonelective 或 profit-sharing 供款。"},
            "after_tax_already": {"en": "After-tax 401(k) contributions already made this year. The page subtracts these from the remaining Mega Backdoor room.", "zh": "本年度已经完成的税后 401(k) 供款。页面会把它从剩余 Mega Backdoor Roth 空间中扣掉。"},
            "catch_up_limit": {"en": "Override if your situation needs a different catch-up amount than the default shown for your age and year.", "zh": "如果你的情况需要与默认年龄/年度规则不同的 catch-up 金额，可在这里覆盖。"},
            "plan_after_tax_cap": {"en": "Some plans cap after-tax contributions below the IRS maximum. Leave at 0 if you do not want to apply a separate plan cap.", "zh": "有些计划对税后供款设置的上限低于 IRS 上限。如果不想额外加这个限制，就保持为 0。"},
            "allows_after_tax": {"en": "Mega Backdoor Roth requires the plan to permit after-tax 401(k) contributions in the first place.", "zh": "Mega Backdoor Roth 的前提是计划本身允许税后 401(k) 供款。"},
            "allows_conversion": {"en": "The contribution step alone is not enough. You also need an in-plan Roth conversion or an in-service rollover path to complete the strategy.", "zh": "只有供款还不够。还需要计划允许 in-plan Roth conversion 或在职转出，才能完成这个策略。"}
        }
    },
    "roadmap_page": {
